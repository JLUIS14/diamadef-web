...
export const Navbar = () => {
  ...
  return (
    <nav className="fixed top-0 left-0 w-full z-50 bg-gradient-to-r from-green-800 to-green-400 shadow-lg px-4 py-2 flex items-center justify-between">
      ...
      {/* Menú */}
      <ul
        className={`flex-col md:flex-row md:flex gap-2 items-center absolute md:static top-20 left-0 w-full md:w-auto bg-green-800 md:bg-transparent transition-all duration-300 z-10 ${
          open ? "flex" : "hidden"
        }`}
      >
        {navLinks.map((link) => {
          if (link.name === "Productos") {
            return (
              ...
              >
                ...
                >
                  Productos{" "}
                  <span className="ml-1 text-[10px] align-middle inline-block translate-y-[1px]">
                    ▼
                  </span>
                </button>
                <ul
                      ...
                      {item.submenu ? (
                        <>
                          <span className="flex justify-between items-center px-4 py-2 cursor-pointer hover:bg-yellow-300">
                            {item.label}
                            <span className="ml-2 text-[10px] leading-none translate-y-[1px]">
                              ▶
                            </span>
                          </span>
                          <ul
                            className={`absolute top-0 left-full w-56 bg-white text-green-900 shadow-xl rounded z-50 transition-all duration-200 ${
